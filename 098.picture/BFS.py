...
str1 = ['','','','','','','','','','','','','','','']
str1[0] = '../098.picture/Road/Road_5.jpg'
...
for qwe in range(0,1):
    ...
    starty = int((width/40)*(3/10))
    print(startx,starty)
    outputs[startx][starty] = 1
    endx = -1
    endy = -1
    ...
    while(head < tail):
        ...
        for i in range(4):
            ...
            plt.imshow(outputs)
            plt.show()
            if comparelbp(tx,ty,que.x[head],que.y[head],image1,image1,0.985,0.98) and book[tx][ty] == 0 :
                book[tx][ty] = 1  # 標記為已走過
                outputs[tx][ty] = 1
                # 插入新的點到佇列中
                que.x[tail] = tx
    ...
    image1 = imgfilter(image1,height,width)
    plt.imshow(image1)
    plt.show()
    ...
    current_marker = 1
    n_markers = 10
#    ret, markers = cv2.connectedComponents(image1)
#    plt.hist(markers.ravel(), 6, [0, 5])
#    plt.show()
    marker_image = image1.copy()
    plt.hist(image1.ravel(), 6, [0, 5])
    plt.show()
    print(image1)
    plt.imshow(img)
    plt.show()   
    plt.imshow(image1)
    plt.show() 
    marker_image_copy = image1.copy()
    img=cv2.imread("../098.picture/Road/Road_5.jpg")
    marker_image_copy = cv2.watershed(img, image1)
    segments = np.zeros(img.shape, dtype = np.uint8)
    plt.hist(marker_image_copy.ravel(), 10, [0, 9])
    plt.show()
